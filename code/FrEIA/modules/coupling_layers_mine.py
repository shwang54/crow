...
from .coeff_functs import F_conv, F_fully_connected, F_GRU, F_GRU_cgate, F_cgate
...
#         print('tuple(range(1, self.ndims+1)): ', tuple(range(1, self.ndims+1)))
#         print('jac: ', jac)
        
        jac_total = torch.sum(jac, dim=tuple(range(1, self.ndims+1)))
#         print('jac_total.shape: ', jac_total.shape)
#         print('jac_total: ', jac_total)
...
#         print('jac.shape before sum: ', jac.shape)
#         print('jac before sum:', jac)
        for i in range(self.ndims-1):
            jac = torch.sum(jac, dim=1)

#         print('jac.shape after sum: ', jac.shape)
#         print('jac after sum:', jac)
...
#         self.sig1 = nn.Linear(self.split_len1, self.split_len1)
#         self.sig1 = nn.ReLU()
        self.sig1 = nn.Sigmoid()
        self.fc2 = nn.Linear(self.split_len2, self.split_len2)
#         self.sig2 = nn.Linear(self.split_len2, self.split_len2)
#         self.sig2 = nn.ReLU()
...
#         print("x[0].shape: ", x[0].shape)
    ...
    def recurrent_block_jacobian(self, x, h_r1, h_r2, rev=False):
        ...
        jac = (torch.sum(self.log_e(s1), dim=1)
               + torch.sum(self.log_e(s2), dim=1)
              + torch.sum(self.sig1(self.fc1(h_r1)), dim=1)
              + torch.sum(self.sig2(self.fc2(h_r2)), dim=1))
...
class glow_gru_cgate_coupling_layer2(nn.Module):
    def __init__(self, dims_in, F_class=F_GRU, F_cgate=F_cgate, F_args={},
                 clamp=5.):
        super(glow_gru_cgate_coupling_layer2, self).__init__()
        channels = dims_in[0][0]
        self.ndims = len(dims_in[0])
...
#         self.sig2 = nn.Sigmoid()
        ...
        self.cg1 = F_cgate(self.split_len1, self.split_len1)
        self.cg2 = F_cgate(self.split_len2, self.split_len2)
...
#             jac = (torch.sum(self.log_e(s1), dim=1)
#                    + torch.sum(self.log_e(s2), dim=1)
#                   + torch.sum(self.cg1(h_r1), dim=1)
#                   + torch.sum(self.cg2(h_r2), dim=1))
        else:
            r1, h_r1_next = self.s1(x1, h_r1)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = (x2 - t1) / self.e(s1)
...
class glow_gru_cgate_coupling_layer3(nn.Module):
    def __init__(self, dims_in, F_class=F_GRU, F_cgate=F_cgate, F_args={},
                 clamp=5.):
        super(glow_gru_cgate_coupling_layer3, self).__init__()
        channels = dims_in[0][0]
        self.ndims = len(dims_in[0])
...
#         print("h_s1.shape: ", h_s1.shape)
        ...
        if not rev:
            ...
            x2 = x2 * self.cg1(h_r2_next)

            r1, h_r1_next = self.s1(y1, h_r1)
            s1, t1 = r1[:, :self.split_len2], r1[:, self.split_len2:]
            y2 = self.e(s1) * x2 + t1
            
            y1 = y1 * self.cg2(h_r1_next)
        else:  # names of x and y are swapped!   
            ...
            x1 = x1 * self.cg2(h_r1_next)
            
            r2, h_r2_next = self.s2(y2, h_r2)
            s2, t2 = r2[:, :self.split_len1], r2[:, self.split_len1:]
            y1 = (x1 - t2) / self.e(s2)
            
            y2 = y2 * self.cg1(h_r2_next)
...
#         print("x[0].shape: ", x[0].shape)
    ...
    def recurrent_block_jacobian(self, x, h_r1, h_r2, rev=False):
        ...
        jac = (torch.sum(self.log_e(s1), dim=1)
               + torch.sum(self.log_e(s2), dim=1)
              + torch.sum(self.cg1(h_r2_next), dim=1)
              + torch.sum(self.cg2(h_r1_next), dim=1))
        for i in range(self.ndims-1):
            jac = torch.sum(jac, dim=1)
